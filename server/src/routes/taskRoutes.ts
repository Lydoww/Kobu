...
router.patch(
  '/task/:id',
  authMiddleware,
  validateRequest(updateTaskValidationRules),
  updateTask
);
 ...
 * /api/task/{id}/move:
 *   patch:
 *     summary: Move a task to a different position/column
 *     tags: [Tasks]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 */
router.patch('/task/:id/move', authMiddleware, moveTask);
router.post('/fix-orders', authMiddleware, fixOrders);
